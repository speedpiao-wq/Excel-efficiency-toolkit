param(
    [string] $OutputPath = (Join-Path (Get-Location) '文本框效率工具-可配置版.xlam'),
    [ValidateRange(1, 409)][double] $DefaultFontSize = 14,
    [string] $DefaultFontHex = '#0033CC',
    [string] $DefaultFillHex = '#FFFFFF',
...
$templateRoot = Join-Path $skillRoot 'assets\configurable-textbox-template'
$modulePath = Join-Path $templateRoot 'vba\modTextBoxFormatter.bas'
$formCodePath = Join-Path $templateRoot 'vba\frmTextBoxSettings-code.txt'
$ribbonPath = Join-Path $templateRoot 'ribbon\customUI.xml'
$workRoot = Join-Path ([IO.Path]::GetTempPath()) 'excel-global-addin-maker'
...
if ($DefaultFontHex -notmatch '^#?[0-9A-Fa-f]{6}$') {
    throw 'DefaultFontHex must contain six hexadecimal digits, for example #0033CC.'
}
if ($DefaultFillHex -notmatch '^#?[0-9A-Fa-f]{6}$') {
    throw 'DefaultFillHex must contain six hexadecimal digits, for example #FFFFFF.'
}
if (-not $DefaultFontHex.StartsWith('#')) { $DefaultFontHex = "#$DefaultFontHex" }
if (-not $DefaultFillHex.StartsWith('#')) { $DefaultFillHex = "#$DefaultFillHex" }
$DefaultFontHex = $DefaultFontHex.ToUpperInvariant()
$DefaultFillHex = $DefaultFillHex.ToUpperInvariant()
...
$moduleSource = $moduleSource -replace '(?m)^Attribute VB_Name = .+\r?\n', ''
$moduleSource = $moduleSource -replace '(?m)^Private Const DEFAULT_FONT_SIZE As Double = .+$', "Private Const DEFAULT_FONT_SIZE As Double = $DefaultFontSize"
$moduleSource = $moduleSource -replace '(?m)^Private Const DEFAULT_FONT_HEX As String = .+$', "Private Const DEFAULT_FONT_HEX As String = `"$DefaultFontHex`""
$moduleSource = $moduleSource -replace '(?m)^Private Const DEFAULT_FILL_HEX As String = .+$', "Private Const DEFAULT_FILL_HEX As String = `"$DefaultFillHex`""
$defaultBoldLiteral = if ($DefaultBold) { 'True' } else { 'False' }
$moduleSource = $moduleSource -replace '(?m)^Private Const DEFAULT_BOLD As Boolean = .+$', "Private Const DEFAULT_BOLD As Boolean = $defaultBoldLiteral"
...
try {
    ...
    if ($null -eq $workbook.VBProject) {
        throw 'Excel blocked access to the VBA project. Enable Trust access to the VBA project object model only while building the add-in.'
    }
    $moduleComponent = $workbook.VBProject.VBComponents.Add(1)
    $moduleComponent.Name = 'modTextBoxFormatter'
    $moduleComponent.CodeModule.AddFromString($moduleSource)
